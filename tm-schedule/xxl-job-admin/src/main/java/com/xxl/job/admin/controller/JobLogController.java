...
public class JobLogController {
    ...
    public Map<String, Object> pageList(@RequestParam(required = false, defaultValue = "0") int start,
                                        ...
                                        int triggerCode, String name,
                                        @RequestParam(required = false) String scheduleids) {
        ...
        //delete by ref.tian 20190520
//        List<String> permissionUsersList = null;
//        if (permissionUsers != null && permissionUsers.length > 0) {
//            permissionUsersList = new ArrayList<>();
//            for (String user :
//                    permissionUsers) {
//                permissionUsersList.add(user);
//            }
//        }
        ...
        // page query
        List<XxlJobLog> list = xxlJobLogDao.pageList(start, length, jobGroup, jobId, triggerTimeStart, triggerTimeEnd, logStatus, triggerCode, name,scheduleids);
        int list_count = xxlJobLogDao.pageListCount(start, length, jobGroup, jobId, triggerTimeStart, triggerTimeEnd, logStatus,triggerCode,name,scheduleids);
    ...
    @ResponseBody
    @PermessionLimit(limit = false)
    public Map<String, Object> load(@PathVariable("id") int logId) {
        XxlJobLog jobLog = xxlJobLogDao.load(logId);
        Map<String, Object> maps = new HashMap<String, Object>();
        maps.put("data", jobLog);
        return maps;
    }

    @RequestMapping(value = "/logdetail/{id}", method = RequestMethod.GET)
    @ResponseBody
    @PermessionLimit(limit = false)
    public ReturnT<LogResult> logDetail(@PathVariable("id") int logId) {
        XxlJobLog jobLog = xxlJobLogDao.load(logId);
        return logDetailCat(jobLog.getExecutorAddress(), jobLog.getTriggerTime().getTime(), logId, 1);
    }
