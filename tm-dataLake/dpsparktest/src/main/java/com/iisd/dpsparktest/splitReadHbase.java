...
public class splitReadHbase extends DPSparkBase implements Serializable {
    @Override
    public void main(Map<String, String> arrm) throws Exception {
...
//        scan.setFilter(filterList);
        ...
        JavaPairRDD<ImmutableBytesWritable, Result> myRDD = DPHbase.saltRddRead("sys_group_user20190710102823", "1558058400000", "1558062000000", scan);
        ...
        stuDf.printSchema();
        stuDf.createOrReplaceTempView("mfg_assembly_detail20190521103901");

//            Dataset<Row> nameDf = spark.sql("select * from mfg_assembly_detail order by scantime_stamp desc LIMIT 20");
        Dataset<Row> nameDf = spark.sql("select count(*) from mfg_assembly_detail20190521103901");
        nameDf.show();
